...
Use fpca to impute missing values


```{r}
Gdf = Gdf %>%
  select(-sumfun, -csr) %>%
  pivot_wider(names_from = r, values_from = fundiff,
              names_prefix = "r_")

# define matrix for refund
G_mat = Gdf %>% select(starts_with("r_")) %>% as.matrix()

# apply fpca
fpc_obj = fpca.face(as.matrix(G_mat),knots = 40)
      
# impute missing values using FPCA fitted values
Gdf = Gdf %>%
      # convert to long format
      pivot_longer(contains("r_"), names_to = "index", 
                   values_to = "value") %>%
...
lmat <- I(matrix(1/J, ncol= J, nrow= I))
### tmat: time indices
...
p_qt = as_refundObj(Gmat_qt) %>% ggplot(aes(index, value)) +
...
Plot the data, transformed and not.

```{r}
p_postImpute = Gdf %>%
  pivot_longer(contains("r_"), names_to = "r", 
                   values_to = "value",
               names_prefix = "r_",
...
fit_afc <- gam(survival_days ~ stage + adjuvant_therapy +
                       ti(t_int, G, by=l_int, bs=c("cr","cr"), k=c(10,10), mc=c(FALSE,TRUE)), weights=survival_status,
