...
extract_surface(Gdf, ovarian_FDA@`Functional Cox`$Gest$fit_lfcm, analysis_vars = c("age"), se = TRUE) %>%
...
bind_rows(extract_surface(Gdf, ovarian_FDA@`Functional Cox`$Gest$fit_afcm, analysis_vars = c("age")) %>% 
            mutate(type = "AFCM surface"),
          extract_surface(Gdf, ovarian_FDA@`Functional Cox`$Gest$fit_afcm, analysis_vars = c("age"), se = TRUE) %>% 
            mutate(type = "p < 0.05")) %>%
  mutate(hr = exp(value)) %>%
...
fit_afcm = ovarian_FDA@`Functional Cox`$Gest$fit_afcm
fit_lfcm = ovarian_FDA@`Functional Cox`$Gest$fit_lfcm
