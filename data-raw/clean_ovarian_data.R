...
add_clustering <- function(mximg){
  ...
  # create ppp object
  pp_obj = ppp(mximg[["x"]], mximg[["y"]], window = w, checkdup = FALSE)

  if(mximg[["cluster"]][1] == TRUE){
    pp_obj_clust = rMatClust(7, 1, 20, w)
    pp_obj_clust2 = rMatClust(4, 5, 15, w)
    pp_obj_clust = superimpose(pp_obj_clust2,pp_obj_clust)
  }else{
    pp_obj_clust = spatstat.random::rpoispp(50, win = w)
  }

  pp_obj = superimpose(pp_obj,pp_obj_clust)
...
ovarian_gfun = extract_summary_functions(ovarian_df, "patient_id",
